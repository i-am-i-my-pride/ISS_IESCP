		...
		init : function() {
			var vm = new Vue( {
                ...
                data: {
                	updateStatus:true,
                	filter: {
                		CPeopleName: "",CHospitalName: "",CMobile: "",CDeptName: "",COrderStatus: ""
                	},
                	userFilter: {
                		CName: "",CMobile: ""
                	},
    				...
    				importData: function() {
    					issapi.importData({
	    					...
	    					multipart_params: { // 附加参数
	    						method: "abroadMgrAction.importData",
	    						cols: "CPeopleName,CMobile,CCertfCls,CCertfCde,CHospitalName,CDeptName,TUsedTm,TTreatment,TCancelDate,COrderStatus,CDesc,CCancelReason",
	            				titles: "就诊人姓名,就诊人手机号,证件类型,证件号码,医院名称,科室名称,使用时间,就诊时间,取消时间,预约类型,备注,取消原因"
	    					},
	    					template: "template-cus-abroad.xls", // 指定模板名称供下载
	    					completeMsg: "客户海外转诊导入完成" // 导入完成后的提示文本
    				...
    				exportData: function() {
    					issapi.exportData( {
	            			...
	            			data: $.extend({ // titles用作导出excel的标题，cols用作从数据表中读取数据
		            				cols: ["CPeopleName", "CMobile", "CCertfCls", "CCertfCde", "CHospitalName", "CDeptName",
		            					"TUsedTm", "TTreatment","TCancelDate", "COrderStatus","CDesc","CCancelReason"],
		            				titles: ["就诊人姓名", "手机号", "证件类型", "证件号码",  "医院名称", "科室名称", "使用时间","就诊时间", "取消时间", "预约类型","备注","取消原因"]
		            			}, 
		            			vm.filter // 用作数据查询的条件
	            			),
	        				pageno: 1,//vm.$refs.cusAbroad.pageno, // 从第几页开始导出
	        				pagesize: 10000,//vm.$refs.cusAbroad.pagesize // 导出记录条数
	            		} );
    				},
